...
public class FuntionsElementsTask implements Task {
    @Override
    @Step("{0} selección del ID card ELEMENTS")
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(Scroll.to(ID_CARD_ELEMENTS));
        actor.attemptsTo(WaitUntil.the(ID_CARD_ELEMENTS, isEnabled()).forNoMoreThan(TIME_SHORT).seconds(),
                Click.on(ID_CARD_ELEMENTS));
    }

    //encapsulamiento de la clase generando un método
